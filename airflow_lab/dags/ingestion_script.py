...
def ingest_data(table_name,pq_file):
    ...
    trips_data = pq.read_table(pq_file)
    trips = trips_data.to_pandas()

    engine = create_engine(f'postgresql://root:root@de_postgres:5432/ny_taxi')
    trips = trips.rename(columns={'airport_fee': 'airport_Fee'})
    ...
    for batch in parquet_file.iter_batches(batch_size = 100000):
        ...
        batch_df = batch_df.rename(columns={'airport_fee': 'airport_Fee'})

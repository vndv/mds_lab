...
resource "yandex_storage_bucket" "mds_lab" {
  access_key    = yandex_iam_service_account_static_access_key.sa-static-key.access_key
  secret_key    = yandex_iam_service_account_static_access_key.sa-static-key.secret_key
  bucket_prefix = "mds-lab"
  acl           = "public-read"
...
resource "yandex_compute_instance" "mssql" {
  name        = "mssql"
  platform_id = "standard-v3"
  zone        = yandex_vpc_subnet.default_subnet.zone
  ...
  boot_disk {
    auto_delete = true
    initialize_params {
      image_id = "fd8fvanr8qlctmsa466g" # with sqlserver installed
      size     = 30
      type     = "network-ssd"
    }
  }
